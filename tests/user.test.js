...
test('Should signup a new user', async () => {
  const response = await request(app)
    ...
    .send({
      name: 'Nicolas',
      email: 'nicolas_desmedt1@example.com',
      password: 'mypass123',
    })
    .expect(201);
...
test('Should upload avatar image', async () => {
  await request(app)
    ...
    .expect(200);
  const user = await User.findById(userOneId);
  expect(user.avatar).toEqual(expect.any(Buffer)); // To equal vereist geen === -> objects versch in mem maar zelfde prop zijn gelijk
});
